...
def main(argv):
    ...
    #Load Input File
    inputFile = []
    with open(PATH + '/DEC_input.txt', 'r') as txt:
        for line in txt.readlines():
            inputFile.append(line.rstrip())

    #Write File
    with open(PATH + '/DEC_output.txt', 'w') as txt:
        for ndx, member in enumerate(inputFile):
            txt.write(decrypt9(decrypt9(decrypt9(inputFile[ndx], keyFile[0], alpha), keyFile[0], alpha), keyFile[0], alpha))
        txt.close()
